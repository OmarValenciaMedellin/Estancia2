...
// Incluye el modelo de reportes
include_once "app/models/reportModel.php";

// Incluye la librería FPDF para generar PDFs
...
{
    private $model; // Modelo para consultas a BD

    public function __construct($connection)
    {
        // Crea instancia del modelo con la conexión
    ...
    {
        // Trae datos generales de la cotización
        $general = $this->model->consultarCotizacionPorId($id);

        // Trae detalle de materiales de esa cotización
        ...
        // Filas de la tabla (recorre cada material)
        ...
        // Muestra PDF en el navegador
        $pdf->Output();
        exit;
    }

    // =====================================================
    // 2) PDF general de cotizaciones (filtrado)
    // =====================================================
    public function pdfGeneralCotizaciones()
    {
        // Obtiene filtros desde GET (o pone valores por defecto)
        $inicio    = $_GET['inicio'] ?? date("Y-m-01");
        $fin       = $_GET['fin'] ?? date("Y-m-d");
        $clienteId = $_GET['cliente'] ?? null;

        // Consulta cotizaciones filtradas
        $cotizaciones = $this->model->consultarCotizacionesFiltradas($inicio, $fin, $clienteId);

        // Variables para contar y sumar totales
        $totalCotizaciones = 0;
        $sumaSubtotal      = 0;
        $sumaDescuento     = 0;
        $sumaImpuestos     = 0;
        ...
        // Datos del periodo consultado
        $pdf->SetFont("Arial", "", 10);
        $pdf->Cell(0, 6, "Periodo consultado: $inicio al $fin", 0, 1);
        $pdf->Cell(0, 6, "Fecha de generacion: " . date("Y-m-d H:i"), 0, 1);

        // Texto de filtro por cliente
        $filtroClienteTxt = $clienteId
            ? "Cliente filtrado: ID $clienteId"
            : "Cliente filtrado: TODOS";
        ...
        // Recorre cotizaciones para imprimir filas
        while ($row = $cotizaciones->fetch_assoc()) {
            ...
            // Notas recortadas para que no se encimen
            $concepto = $row['Notas'] ?? '';
            $concepto = mb_substr($concepto, 0, 35);

            // Imprime fila con los mismos anchos
    ...
    {
        // Obtiene totales vendidos por material
        ...
        // Recorre materiales y suma totales
        $suma = 0;
        foreach ($data as $row) {
            $material = $row[0];
            $total    = (float) $row[1];
